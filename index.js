...
function reverseCharacters(characters) {
  ...
  if (typeof characters === "string") {
        change = characters.split('').reverse().join('')
    } else {
    change = String(characters).split('').reverse().join('');
    Number(change);
   }
    return change
...
function reverseArrayItems(array) {
  ...
  for (i = 0; i < array.length; i++) {
    reverseString = reverseCharacters(array[i]);
    emptyArray.push(reverseString);
  }
  return emptyArray;
}
...
console.log(reverseArrayItems(arrayTest1));
console.log(reverseArrayItems(arrayTest2));
